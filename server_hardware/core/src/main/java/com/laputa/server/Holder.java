...
public class Holder implements Closeable {
    ...
    public Holder(ServerProperties serverProperties, boolean restore) {
        ...
        this.eventorProcessor = new EventorProcessor( blockingIOProcessor, stats);
        ...
        String contactEmail = serverProperties.getProperty("contact.email", "name");
        this.sslContextHolder = new SslContextHolder(props, contactEmail);
    }

    //for tests only
    public Holder(ServerProperties serverProperties, String dbFileName) {
        disableNettyLeakDetector();
        this.props = serverProperties;
