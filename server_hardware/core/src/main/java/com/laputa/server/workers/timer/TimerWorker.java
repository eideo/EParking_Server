package com.laputa.server.workers.timer;
...
import com.laputa.server.core.model.widgets.others.eventor.TimerTime;
    ...
    private void process(DashBoard dash, TimerKey key, BaseAction[] actions, long now) {
        for (BaseAction action : actions) {
            if (action instanceof SetPinAction) {
                ...
                EventorProcessor.push( dash, notifyAction.message);
            }
